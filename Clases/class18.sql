...
END
 ...
 v_out is the result of a query that also gets the count of rental_id from the table inventory,
 matching the inventory_id with the one passed in parameters, but with the filter that return_date is null (film not returned).
 If v_out is greater than 0, returns FALSE, otherwise, TRUE.
*/

-- Examples of usage
SELECT inventory_in_stock(23); -- This one throws 1 (TRUE, because MySQL detects boolean as 0 for FALSE and 1 for TRUE)
...
-- Example of usage
CALL film_in_stock(2,2,@myresult);
SELECT @myresult;